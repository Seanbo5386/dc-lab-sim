...
describe("CommandParser", () => {
  ...
  describe("Flag Parsing", () => {
    ...
    describe("negative numbers as values, not flags (F4)", () => {
      it("treats a negative integer as the value of the preceding flag", () => {
        const result = parse("nvidia-smi -i -5");
        expect(result.flags.get("i")).toBe("-5");
        // -5 must NOT be misparsed into a long option like "--5"
        expect(result.flags.has("5")).toBe(false);
      });

      it("treats a negative decimal as a value", () => {
        const result = parse("cmd -t -1.5");
        expect(result.flags.get("t")).toBe("-1.5");
      });

      it("still treats genuine short options as flags", () => {
        const result = parse("nvidia-smi -L");
