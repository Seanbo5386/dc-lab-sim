...
export class NvidiaBugReportSimulator extends BaseSimulator {
  ...
  getMetadata(): SimulatorMetadata {
    return {
      ...
      commands: [
        { name: 'nvidia-bug-report.sh', description: 'Generate comprehensive NVIDIA diagnostic report' },
      ],
    };
  }
  ...
  execute(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    const outputFile = this.getFlagString(parsed, ['output-file', 'o'], '/tmp/nvidia-bug-report.log.gz');
  ...
  private generateReport(
    ...
    if (extra) {
      sections.push('lspci verbose', 'dmidecode', 'kernel modules', 'boot parameters');
    }
    ...
    const healthyGpus = node.gpus.filter((g: GPU) => g.healthStatus === 'OK').length;
    const warningGpus = node.gpus.filter((g: GPU) => g.healthStatus === 'Warning').length;
    const errorGpus = node.gpus.filter((g: GPU) => g.healthStatus === 'Critical').length;
    output += `  Healthy:           ${healthyGpus}\n`;
    if (warningGpus > 0) output += `  \x1b[33mWarning:           ${warningGpus}\x1b[0m\n`;
    if (errorGpus > 0) output += `  \x1b[31mError:             ${errorGpus}\x1b[0m\n`;
    output += `\n`;

    // GPU details
    output += `\x1b[1mGPU Details:\x1b[0m\n`;
    node.gpus.forEach((gpu: GPU, idx: number) => {
      const statusColor = gpu.healthStatus === 'OK' ? '\x1b[32m' :
                         gpu.healthStatus === 'Warning' ? '\x1b[33m' : '\x1b[31m';
    ...
    const totalNvlinks = node.gpus.reduce((sum: number, g: GPU) => sum + g.nvlinks.length, 0);
    const activeNvlinks = node.gpus.reduce((sum: number, g: GPU) => sum + g.nvlinks.filter(l => l.status === 'Active').length, 0);
    ...
    const totalSingleBit = node.gpus.reduce((sum: number, g: GPU) => sum + g.eccErrors.singleBit, 0);
    const totalDoubleBit = node.gpus.reduce((sum: number, g: GPU) => sum + g.eccErrors.doubleBit, 0);
    ...
    if (allXidErrors.length === 0) {
      ...
      Object.entries(xidCounts).forEach(([code, count]) => {
        ...
        const severityColor = xidInfo.severity === 'critical' ? '\x1b[31m' :
                             xidInfo.severity === 'warning' ? '\x1b[33m' : '\x1b[0m';
  ...
  private getXidInfo(code: number): { severity: string; description: string } {
    const xidDatabase: Record<number, { severity: string; description: string }> = {
      13: { severity: 'warning', description: 'Graphics Engine Exception' },
      31: { severity: 'critical', description: 'GPU memory page fault' },
      32: { severity: 'warning', description: 'Invalid or corrupted push buffer' },
      ...
      61: { severity: 'warning', description: 'Internal micro-controller breakpoint' },
      62: { severity: 'warning', description: 'Internal micro-controller halt' },
      63: { severity: 'warning', description: 'ECC page retirement / Row remapping' },
      64: { severity: 'warning', description: 'ECC page retirement / Row remapping' },
      68: { severity: 'warning', description: 'Video processor exception' },
      69: { severity: 'warning', description: 'Graphics engine class error' },
      72: { severity: 'critical', description: 'NVLink error - Link training failed' },
      ...
      78: { severity: 'critical', description: 'NVLink flow control CRC error' },
      79: { severity: 'critical', description: 'GPU has fallen off the bus' },
      92: { severity: 'critical', description: 'High single-bit ECC error rate' },
    ...
    return xidDatabase[code] || { severity: 'info', description: 'Unknown error' };
  }
  ...
  private generateRecommendations(node: DGXNode): string[] {
    ...
    const criticalXids = node.gpus.flatMap((g: GPU) => g.xidErrors).filter(
      (xid) => [31, 43, 48, 72, 74, 76, 77, 78, 79, 92, 94, 95, 119, 120].includes(xid.code)
    );
    if (criticalXids.length > 0) {
      recommendations.push('Critical XID errors detected. Review error history and consider GPU RMA if persistent.');
    }

    // Check for ECC errors
    const hasDoubleBitEcc = node.gpus.some((g: GPU) => g.eccErrors.doubleBit > 0);
    if (hasDoubleBitEcc) {
      recommendations.push('Uncorrectable ECC errors detected. Schedule maintenance or replacement.');
    }

    // Check for inactive NVLinks
    const inactiveNvlinks = node.gpus.some((g: GPU) => g.nvlinks.some(l => l.status !== 'Active'));
    if (inactiveNvlinks) {
      recommendations.push('Inactive NVLinks detected. Check fabric manager status and NVSwitch health.');
    }

    // Check for high temperatures
    const highTempGpus = node.gpus.filter((g: GPU) => g.temperature > 80);
    if (highTempGpus.length > 0) {
      recommendations.push(`GPU(s) running hot (>80°C). Check cooling system and airflow.`);
    }

    // Check for power issues
    const powerIssues = node.gpus.filter((g: GPU) => g.powerDraw > g.powerLimit * 0.95);
    if (powerIssues.length > 0) {
      recommendations.push('GPU(s) near power limit. Consider power management adjustments.');
    }

    // Check for unhealthy GPUs
    const unhealthyGpus = node.gpus.filter((g: GPU) => g.healthStatus !== 'OK');
    if (unhealthyGpus.length > 0) {
      recommendations.push(`${unhealthyGpus.length} GPU(s) in non-OK state. Run dcgmi diag for detailed diagnostics.`);
    }
