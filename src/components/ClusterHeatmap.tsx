...
export const ClusterHeatmap: React.FC<ClusterHeatmapProps> = ({
  ...
  const [hoveredGPU, setHoveredGPU] = useState<{
    nodeId: string;
    gpu: GPU;
  } | null>(null);
            ...
            <div key={node.id} className="flex items-center gap-3">
              ...
              <div className="flex gap-1 flex-wrap">
                {node.gpus.map((gpu) => {
                  ...
                  const isHovered =
                    hoveredGPU?.nodeId === node.id &&
                    hoveredGPU?.gpu.id === gpu.id;
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={() =>
                        setHoveredGPU({ nodeId: node.id, gpu })
                      }
        ...
        <div className="mt-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <span
              ...
              style={{
                color: config.getColor(config.getValue(hoveredGPU.gpu)),
              }}
            >
              {config.getValue(hoveredGPU.gpu).toFixed(1)}
              {config.unit}
            </span>
          </div>
          <div className="grid grid-cols-4 gap-2 mt-2 text-xs">
            <div>
              ...
              <span className="text-gray-300">
                {Math.round(hoveredGPU.gpu.utilization)}%
              </span>
            </div>
            <div>
              <span className="text-gray-500">Temp:</span>{" "}
              <span className="text-gray-300">
                {Math.round(hoveredGPU.gpu.temperature)}°C
              </span>
            </div>
            <div>
              <span className="text-gray-500">Power:</span>{" "}
              <span className="text-gray-300">
                {Math.round(hoveredGPU.gpu.powerDraw)}W
              </span>
            </div>
            <div>
