...
test.describe("Integration Workflows", () => {
  ...
  test.describe("Production Readiness Workflow", () => {
    test("should complete full production validation workflow", async ({
      page,
    }) => {
      const helper = await createHelper(page);
  ...
  test.describe("Cross-tool validation", () => {
    test("should correlate ClusterKit results with nvidia-smi", async ({
      page,
    }) => {
      const helper = await createHelper(page);
  ...
  test.describe("Sequential command execution", () => {
    test("should handle long command sequences", async ({ page }) => {
      ...
      for (const cmd of commands) {
        ...
        await helper.waitForCommandOutput(
          cmd.includes("burn-in") ? 15000 : 5000,
        );
      }
    ...
    test("should handle alternating command types", async ({ page }) => {
      ...
      // Alternate between GPU and network commands. Check the nvidia-smi
      // output right after it runs — xterm.js only renders visible rows in
      // the DOM (no off-screen scrollback), so by the time three more
      // sizable command outputs have printed, the earlier "GPU" text has
      // scrolled out of what getTerminalOutput() can see.
      await helper.typeCommand("nvidia-smi");
      await helper.waitForCommandOutput();
      const nvidiaSmiOutput = await helper.getTerminalOutput();
      expect(nvidiaSmiOutput).toContain("GPU");
  ...
  test.describe("Performance and stress tests", () => {
    ...
    test("should handle very long output without breaking UI", async ({
      page,
    }) => {
      const helper = await createHelper(page);
